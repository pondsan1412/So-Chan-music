...
class AudioController(object):
    ...
    def make_view(self):
        ...
        pause_button = MusicButton(
            lambda _: self.pause(),
            emoji=f"{config.anime_girl_dance}" if self.guild.voice_client.is_playing() else "▶️",
        ...
        joke_sochan = MusicButton(
            callback=None,
            url="https://www.facebook.com/SoChanbot",
            row=2,
            emoji=f"{config.cute_bunny}",
            label="สนับสนุนเราด้วยการกดติดตามเพจ",
            style=discord.ButtonStyle.gray,
            )
    
        view.add_item(joke_sochan)
        self.last_view = view
